...
if( ! $post_thumbnail_id && ! empty( $placeholder_enabled ) ){
	$img_markup = str_replace('woocommerce-placeholder', 'woocommerce-placeholder ' . $html_class, wc_placeholder_img( $size ) );
...
if( in_array( $click_action, array( 'product_page', 'product_page_new' ) ) ){
	...
	echo '<div class="'. $html_class .'" data-wcpt-image-size="'. $size .'" '. $title .' '. $lightbox_attrs .' '. $zoom_attrs .' >' . $img_markup . $lightbox_icon . '</span>';
}
