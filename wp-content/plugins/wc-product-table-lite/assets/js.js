jQuery(function($){
  ...
  function get_table($wcpt){
    var device = get_device($wcpt),
        table_selector = '.wcpt-table-scroll-wrapper-outer.wcpt-device-laptop:visible > .wcpt-table-scroll-wrapper > .wcpt-table:not(.wcpt-left-freeze):not(.wcpt-right-freeze)';
    ...
    return $wcpt.find(table_selector.replace('laptop', device));
  }
  ...
  $('body').on('wcpt_layout', '.wcpt', function layout(){
    var $wcpt = $(this),
        ...
        $wrap_outer = $wrap.parent(),
        $table = $wrap.find('.wcpt-table').not('.wcpt-right-freeze, .wcpt-left-freeze');
    ...
    // maybe overflow
    if( $wrap.innerWidth() >= $table.outerWidth() ){
      $wrap_outer.addClass('wcpt-overflow-visible');
      $wrap_outer.removeClass('wcpt-top-scroll-enabled');
    }else{
      $wrap_outer.removeClass('wcpt-overflow-visible');
      $wrap_outer.addClass('wcpt-top-scroll-enabled');
    }

    if(navigator.userAgent.toLowerCase().indexOf('firefox') == -1){ // non-firefox
      // top scrollbar sync
      var $top_scroll = $wrap_outer.find('.wcpt-top-scroller');
      $top_scroll.find('.wcpt-top-scrollbar').width( $table.outerWidth() );

      $top_scroll
        .on('mousedown touchstart', function(){
          $wrap.off('scroll.wcpt');
          $top_scroll.off('scroll.wcpt').on('scroll.wcpt', function(){
            $wrap.scrollLeft($(this).scrollLeft());
          });
        })
        .on('mouseup touchend', function(){
          $top_scroll.off('scroll.wcpt');
          $wrap.off('scroll.wcpt').on('scroll.wcpt', function(){
            $top_scroll.scrollLeft($(this).scrollLeft());
          })
        })
        .on('keyup', function(e){ // keydown
          if ( [37, 39].indexOf( e.keyCode ) !== -1 ) { // left arrow
            $wrap.scrollLeft($(this).scrollLeft());
          }
        })
      ...
      $wcpt.addClass('wcpt-br-ff'); // hide top scroll
    ...
    var sc_attrs_string = $wcpt.attr( 'data-wcpt-sc-attrs' ),
        ...
        freeze = {
          'laptop': {
            'left' : ( typeof sc_attrs.laptop_freeze_left == 'undefined' || sc_attrs.laptop_freeze_left == ''  ) ? 0 : parseInt( sc_attrs.laptop_freeze_left ),
            'right' : ( typeof sc_attrs.laptop_freeze_right == 'undefined' || sc_attrs.laptop_freeze_right == ''  ) ? 0 : parseInt( sc_attrs.laptop_freeze_right ),
          },
          'tablet': {
            'left' : ( typeof sc_attrs.tablet_freeze_left == 'undefined' || sc_attrs.tablet_freeze_left == ''  ) ? 0 : parseInt( sc_attrs.tablet_freeze_left ),
            'right' : ( typeof sc_attrs.tablet_freeze_right == 'undefined' || sc_attrs.tablet_freeze_right == ''  ) ? 0 : parseInt( sc_attrs.tablet_freeze_right ),
          },
          'phone': {
            'left' : ( typeof sc_attrs.phone_freeze_left == 'undefined' || sc_attrs.phone_freeze_left == ''  ) ? 0 : parseInt( sc_attrs.phone_freeze_left ),
            'right' : ( typeof sc_attrs.phone_freeze_right == 'undefined' || sc_attrs.phone_freeze_right == ''  ) ? 0 : parseInt( sc_attrs.phone_freeze_right ),
          },
        },
        device = get_device($wcpt),
        $table = get_table($wcpt);
    if(
      typeof( freeze[device] ) !== 'undefined' &&
      ! ( freeze[device]['left'] == 0 && freeze[device]['right'] == 0 )
    ){
      requestAnimationFrame( function(){freeze_columns( freeze[device], $table );} );
    }
  ...
  function after_every_load($container){
    ...
    // ### sync qty input with variation permitted qty input min-max
  ...
  // get row -- avoid freeze tables
  function get_product_row($elm){
    var $row = $elm.closest('.wcpt-row'),
        product_id = $row.attr('data-wcpt-product-id'),
        $table = $elm.closest('.wcpt-table');

    if( $table.is('.wcpt-right-freeze, .wcpt-left-freeze') ){
      $row = $table.siblings('.wcpt-table-scroll-wrapper').find('.wcpt-row[data-wcpt-product-id="'+ product_id +'"]');
    }
  ...
  $('body').on('adding_to_cart', function(e, $button, data){
    ...
    var $row = $button.closest('.wcpt-row');
    if( $row.length ){
  ...
  function button_click(e){
    var $button = $(this),
        ...
        $product_row = get_product_row($button),
        product_id = $product_row.attr('data-wcpt-product-id'),
        is_variable = $product_row.hasClass('wcpt-product-type-variable'),
        has_addons = $product_row.hasClass('wcpt-product-has-addons');
    ...
    var ajax_data = {
      ...
      'add-to-cart' : $product_row.attr('data-wcpt-product-id'),
      'product_id'  : product_id,
      'quantity'    : 1,
    };

    // quantity input
    var $wcpt_qty =  $('.wcpt-quantity-wrapper > input[type=number]', $product_row),
        $wc_qty = $('.cart .qty', $product_row);
    ...
    if( is_variable || has_addons ){
      ...
      if( is_variable ){
        ...
        var $radio_control = $product_row.find('.wcpt-select-variation.wcpt-selected'),
            $dropdown_control = $product_row.find('.wcpt-select-variation-dropdown'),
            $selected_option = $(), // dom elm holds all single variation information
            variation_id = $product_row.data('wcpt_variation_id'),
            complete_match = $product_row.data('wcpt_complete_match'),
            attributes = $product_row.data('wcpt_attributes'),
            variation_found = $product_row.data('wcpt_variation_found'),
            variation_selected = $product_row.data('wcpt_variation_selected'),
            variation_available = $product_row.data('wcpt_variation_available');
            variation_ops = $product_row.data('wcpt_variation_ops');

        // if row has variation selection options, then
        ...
        if( variation_id && complete_match ){
    ...
    if( $button.hasClass('wcpt-disabled') ){
      return;
    }
  ...
  function wcpt_ajax_add_to_cart( $button, ajax_data ){
    var $product_row = $button.closest('.wcpt-row'),
        url = wcpt_i18n.ajax_url;
  ...
  function prep_product_form( $modal, $button, pre_select ){
    ...
    $( '.cart', $modal ).each( function() {
      ...
      }else { // simple product (with addon)
  ...
  function loading_badge_off_button($button){
    $button.find( '.wcpt-cart-badge-refresh' ).remove();
  }
  ...
  function remove_count_badge_from_button($button){
    $button.find( '.wcpt-cart-badge-number' ).remove();
  }
  ...
  function fix_tooltip_position($tooltip){
    // correct position
    var $content      = $tooltip.find(' > .wcpt-dropdown-menu, > .wcpt-tooltip-content'),
  ...
  $('body').on('click', '.wcpt-clear-filter', function(e){
    ...
    if( filter == 'attribute' || filter == 'category' || filter == 'taxonomy' ){
      ...
      $inputs = $navs.find('input[type=search]');
  ...
  function attempt_ajax( $container, new_query, append, purpose ){
    ...
    var query = '';
    if( append ){
      var earlier_query = $container.attr('data-wcpt-query-string');
    ...
    if( sc_attrs && sc_attrs !== '{}' ){
      sc_attrs = sc_attrs.replace(/[\u00A0-\u9999<>\&]/gim, function(i) {
  ...
  function update_cart_items_ajax(){
    ...
    $.post( wcpt_i18n.ajax_url, { action: 'wcpt_cart_widget' }, function(cart_widget){
      $('body').append( $(cart_widget).css({'opacity': '0'}).animate({'opacity': '1'}, 800) );
    })
  }
  ...
  // freeze columns
  function freeze_columns( freeze, $table ){
    var $left_clone = $table.clone().addClass('wcpt-left-freeze'),
        $right_clone = $table.clone().addClass('wcpt-right-freeze'),
        $clones = $left_clone.add($right_clone),
        $scroll_wrapper = $table.closest('.wcpt-table-scroll-wrapper'),
        $freeze_wrapper = $scroll_wrapper.parent('.wcpt-table-scroll-wrapper-outer');

    $freeze_wrapper.children('.wcpt-left-freeze, .wcpt-right-freeze').remove();

    // cancel
    if( $table.outerWidth() <= $scroll_wrapper.width() ){
      return;
    }

    $freeze_wrapper.prepend($clones);

    /* left clone */
    // iterate all heading / body cells
    $.each(freeze, function(key, val){
      var $clone;
      if( key == 'left' ){
        $clone = $left_clone;
      }else{
      ...
      $( '> tbody > tr > td, > thead > tr > th', $clone ).each(function(){
        var $this = $(this),
            index = $this.index(),
            row_index = $this.closest('tr').index(),
            outer = $this.closest('.wcpt-heading-row').length ? 'thead' : 'tbody';

        // remove excess cells
        if(
          key == 'left' && index >= freeze['left'] ||
          key == 'right' && ( $this.nextAll().length ) >= freeze['right']
        ){
          $this.remove();
          return;
        }
        ...
        var $original_row = $( ' > ' + outer + ' > tr:nth-child(' + ( row_index + 1 ) + ')', $table ),
            $original;
        if( key == 'left' ){
          $original = $( '> *:nth-child(' + ( index + 1 ) + ')', $original_row );
        }else{
          $original = $( '> *:nth-last-child(' + ( $this.nextAll().length + 1 ) + ')', $original_row );
        }
        ...
        // copy row height
        $this.parent('tr').outerHeight($original_row[0].getBoundingClientRect().height);
    ...
    $scroll_wrapper.on('scroll', function(){
      var $this = $(this),
          $freeze_wrapper = $this.closest('.wcpt-table-scroll-wrapper-outer');

      $freeze_wrapper.removeClass('wcpt-scroll-0 wcpt-scroll-1');

      if( $this[0].scrollLeft == 0 ){
        $freeze_wrapper.addClass('wcpt-scroll-0');
      }

      if( $this[0].scrollLeft + $this.width() == $table.outerWidth() ){
        $freeze_wrapper.addClass('wcpt-scroll-1');
      }
    })

    $scroll_wrapper.trigger('scroll');
  }

  $(window).on('wheel', function(e){
    var $target = $(e.target),
        $frozen = $target.closest('.wcpt-left-freeze, .wcpt-right-freeze'),
        $wcpt = $target.closest('.wcpt'),
        $scroll_wrapper = $frozen.siblings('.wcpt-table-scroll-wrapper'),
        $scroll_outer = $target.closest('.wcpt-table-scroll-wrapper-outer'),
        delta = 0;

    if( $frozen.length ){ // scrolling on frozen column
      if( e.originalEvent.deltaX < 0 ){ // left dir
        delta = -5;
        e.preventDefault();
      }else if( e.originalEvent.deltaX > 0 ){ // right dir
        delta = 5;
        e.preventDefault();
      }
      $scroll_wrapper.scrollLeft( $scroll_wrapper.scrollLeft() + delta);
      return;
    }

    // prevent back/forward on $scroll_wrapper
    if( "webkitLineBreak" in document.documentElement.style ){ // webkit only
      if( $scroll_outer.hasClass('wcpt-top-scroll-enabled') ){
        if(
          e.originalEvent.deltaX < 0 && $scroll_outer.hasClass('wcpt-scroll-0') || // back
          e.originalEvent.deltaX > 0 && $scroll_outer.hasClass('wcpt-scroll-1') // forward
        ){
          e.preventDefault();
        }
      }
  ...
  // check all
  $('body').on('change', '.wcpt-add-all-to-cart-cb', function(){
  	var $this = $(this),
  		  $table = $this.closest('.wcpt-table'),
        checked = $this.prop('checked');
  	$table.find('.wcpt-add-to-cart-cb').prop('checked', checked);
  })
  ...
  // sync qty input in freeze
  $('body').on('change', '.wcpt-left-freeze input.qty, .wcpt-right-freeze input.qty', function(){
    var $input = $(this),
        $product_row = get_product_row( $input );
    $product_row.find('input.qty').val($input.val());
  })
  ...
  $('body').on('select_variation', '.wcpt-product-type-variable', function(e, data){
    ...
    if( data.variation ){
      //-- input
      var $input = $row.find('.wcpt-quantity-wrapper > input[type=number]');
      ...
      if( $product_image.length && typeof data.variation.image !== 'undefined' ){
        $product_image.attr({
