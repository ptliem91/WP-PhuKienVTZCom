...
function wcpt_set_default_settings() {

  $current_settings = get_option( 'wcpt_settings' );

  if( empty( $current_settings ) || 'null' == $current_settings ){
...
function wcpt_save_table_settings() {
  ...
  if( count( $errors ) ){ // failure
    ...
    update_post_meta( $post_id, 'wcpt_data', $_POST['wcpt_data'] );
    $my_post = array(
        'ID'=> $post_id,
...
function wcpt_settings_page(){
  ...
  $GLOBALS['wcpt_settings'] = json_decode( stripslashes( get_option( 'wcpt_settings', '' ) ), true );
  ?>
  <script>
    var wcpt = {
        model: {},
        view: {},
        controller: {},
        data: <?php echo json_encode( $GLOBALS['wcpt_settings'] ); ?>,
      };
  </script>
  <?php
...
function wcpt_enqueue_scripts (){
  ...
  // scripts
  wp_enqueue_script( 'wcpt',  plugin_dir_url( __FILE__ ) . 'assets/js'. $min .'.js', 'jquery', WCPT_VERSION, true );
...
function wcpt_ajax( ){
  if( ! empty( $_REQUEST['id'] ) ){
    ...
    if( ! empty( $_REQUEST[ $_REQUEST['id'] . '_sc_attrs'] ) ){
      ...
      $permitted = apply_filters('wcpt_permitted_shortcode_attributes', array(
        'offset',
        'limit',
        'category',
        'ids',
...
add_action( 'wp_ajax_wcpt_remove_cart_item', 'wcpt_remove_cart_item' );
add_action( 'wp_ajax_nopriv_wcpt_remove_cart_item', 'wcpt_remove_cart_item' );
function wcpt_remove_cart_item(){
...
function wcpt_shortcode_product_table($atts){
  ...
  $error_message = wcpt_sc_error_checks( $GLOBALS['wcpt_table_data'] );
  ...
  if( $error_message ){
    ...
    $instance = new WC_Shortcode_Product_Table( $atts );
    $markup .= $instance->get_content();
...
function wcpt_get_product_form( $atts ){
  ...
  <div class="wcpt-product-form wcpt-modal" data-wcpt-product-id="<?php echo $product_id; ?>">
    <div class="wcpt-modal-content">
      ...
      <?php
      echo '<span class="wcpt-product-form-title">' . $product->get_title() . ' – ' . $product->get_price_html() . '</span>';
...
function wcpt_icon( $icon_name, $html_class= '', $style= null, $tooltip='' ){
  ...
  if( file_exists( $icon_file ) ){
    ...
    echo '<span class="wcpt-icon wcpt-icon-'. $icon_name .' '. $html_class .' '. $tooltip_html_class .'" '. $style .'>';
...
function wcpt_check_sort_match( $set_1, $set_2 ){
  ...
  if( $set_1['meta_key'] == $set_2['meta_key'] ){
    return true;
  }
}
  ...
  if( ! class_exists( 'WCPT_Taxonomy_Walker' ) ){
    class WCPT_Taxonomy_Walker extends Walker {
      ...
      function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ){
        ...
        $children = array_diff( get_term_children( $category->term_taxonomy_id, $this->args['taxonomy'] ), $this->args['exclude'] );
        $children = get_terms(array(
          'taxonomy' => $this->args['taxonomy'],
          'hide_empty' => $this->args['hide_empty'],
          'exclude' => $this->args['exclude'],
          'hierarchical' => true,
          'child_of' => $category->term_taxonomy_id,
          'fields' => 'ids',
        ));
        ...
        <div
				...
				>
          <label class="<?php echo $checked ? 'wcpt-active' : ''; ?>" data-wcpt-value="<?php echo $category->term_taxonomy_id; ?>">
            <input
...
// condition
require_once( WCPT_PLUGIN_PATH . 'condition-functions.php' );

// speed test
function wcpt_microtime(){
...
// add_action('wp_footer', 'wcpt_add_selected_to_cart');
function wcpt_add_selected_to_cart (){
  ?>
<div class="wcpt-add-selected">Add selected (4) <?php wcpt_icon('shopping-cart'); ?></div>
  <?php
}
...
function wcpt_shortcode_column( $column, $post_id ) {
  switch ( $column ) {
    case 'shortcode' :
      ?>
      <input style="width: 200px" value="<?php esc_html_e( '[product_table id="'. $post_id .'"]' ); ?>" onClick="this.setSelectionRange(0, this.value.length)" readonly />
      <?php
      break;
  }
}
...
// get varaitions array for the product
...
function wcpt_duplicate_post_link( $actions, $post ) {
    ...
    if( defined('WCPT_PRO') ){
		  ...
		  //$actions['duplicate'] = '<span style="color: #999"></span>';
    }
